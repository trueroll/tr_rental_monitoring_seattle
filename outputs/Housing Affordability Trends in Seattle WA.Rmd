---
title: "2022 Monitoring Report: Composition of Rental Housing and Ownership in Seattle, WA"
subtitle: 'Prepared by the City of Seattle’s Office of Housing and Office of Planning & Community Development'
date: "`r Sys.Date()`"
output: 
  html_document:
    toc: true
...
- The number of housing units in Seattle has increased dramatically to meet increasing demand from a growing population. Most new units are created in large buildings with 50 or more units. There has been a small net increase in units in all building sizes.
...
- Rental listing data does not provide strong evidence that 'mom-and-pop' landlords charge lower rents on average. Nor does it provide strong evidence that rents differ significantly by building size, except insomuch as buildings of different sizes are located in different parts of Seattle, or differ in terms of age or unit sizes. No does it provide strong evidence that property owners that live outside of the state of Washington charge higher rents than property owners located nearer to Seattle.

We provide recommendations regarding the future collection and maintenance of real property data.

### Reproduction

Reproduction code and data for this project can be found on [GitHub.](https://github.com/trueroll/seattle_rental_study) Some elements of this project contain intellectual property and sensitive business information, and have been masked. Each graph has a graph number noted in its caption. These correspond to tabular data stored in the repository.

# Broad trends in Seattle housing costs

The sections below outlines broad trends in Seattle population and housing. For this analysis we used the [US Census Bureau's Individual Public Use Micro Data Sets (IPUMS)](https://usa.ipums.org/usa/). IPUMS are observations of individual Census respondents over time. We aggregated data from the American Community Survey's 5-year estimates from 2005 to 2020, selecting important variables that shed light on Seattle's housing market. 

## Population pressure may be increasing rents

The population of Seattle, WA has grown rapidly over the past decade, from 664,000 in 2010 to 743,000 in 2020. Rents have followed, with median rent 25% higher in 2020 than in 2010. Households' incomes have also increased - median income rose from \$64,000 to \$90,000 over the decade. Increases in population and rents raise legitimate concern over housing affordability in Seattle. The graph below shows these changes in Seattle. Values are indexed to show the relationship in trends rather than levels. 
...
Due to differences in the markets for properties-to-own and properties-to-rent, rental costs tend to track the incomes of renters more closely than owner costs do the incomes of owners. This is one reason policymakers may be more concerned about apartment rents than owner costs. However, it is important to remember that properties-to-own and properties-to-rent are substitutes in consumption. Increasing rents will also increase the purchase price of homes, and high purchase prices for properties-to-own will put upward pressure on rents. Increasing the production of properties-to-own, on the other hand, will put downward pressure on the cost of properties-to-rent.
...
This sections describes the current state and changes over time in the composition of rental housing stock in Seattle. Following the categorization schema used by the US Census Bureau, we focus on the gain of units in five different size classes, from single-family to large buildings with more than 50 units. For this analysis, we rely on a number of data sets. Each sub-section highlights results from a specific data set. Overall, we see little evidence that Seattle is losing units within any category of building size. Most new units are being built in large buildings. We see the most demolition activity within single-family homes, with the number of construction permits is larger than the number of demolition permits.

## Census data

IPUMS data captures [the number of units in respondents' structure](https://usa.ipums.org/usa-action/variables/UNITSSTR#description_section). Using this field, we can count the number of households over time by building size. The graph below shows that nearly all of the net the increases in housing in Seattle is occurring in large buildings with 50 or more units. Mathematically, this will result in a smaller share of units in smaller buildings overall. The number of households living in smaller apartment buildings and single family homes has stayed relatively stable since 2005.   
...
The City of Seattle's Office of Planning and Community Development tracks and reports building permit data [in a well-built online dashboard](https://experience.arcgis.com/experience/53932efb64da40f6914c37e086b7dcbb). Since building permits are an integral part of multiple, highly visible administrative processes, we believe the permit data is reliable and mostly accurate. However, since building permits are not designed to track housing stock over time, they do not capture all the information one might want for the purpose of this analysis. For example, the Office of Planning and Community Development has invested significant time curating permit data, joining it against Assessor parcel layers to bring in additional information about the property being modified. In the data, however, they do not capture a full picture of the structures being modified, or the structure that eventually was constructed. This is not an oversight - such data is outside of the purview of that office, and would be unnecessary for their purposes. But it does make the data much less useful for our particular use case. 

The graph below shows net changes in units categorized by building size. This data is very consistent with the census data. 

```{r fig4.1, out.width = '100%'}
load('G4.1.gph'); G4.1
```

Seattle's permit data also allows us to track gross additions and demolitions separately. The graph below shows these figures over the entire sixteen year period, grouped by building size. There have been very few permits for demolitions of multi-unit buildings in Seattle. Tabular data can be found on GitHub. 
...
In the table below, owners must meet all of the criteria in the first two categories to be categorized as either a "Mom-and-pop" or "Small property manager" owner. The buildings field was not always populated because of the way the King County Assessor tracks buildings data. Owners with null building counts that met the other criteria for "Mom-and-pop" and "Small" landlords were included in those categories. Owners meeting any of the criteria in the next two are categorized as one of "Medium" or "Large" property managers. Owners that meet criteria in both the "Medium" and "Large" property manager categories are categorized as "Large property managers." Finally, we excluded the owners of condominiums from this part of our analysis. 

```{r table5.1, results = 'asis'}
data.frame(
  `Owner type` = c("Mom-and-pop", "Small property manager", "Medium property manager ", "Large property manager")
...
The table below shows owner and unit counts for 2021 that use this categorization. The total number of units in our ownership data in 2020 was 190,393. According the IPUMS, the number of renting households was 186,406. The close agreement in unit counts gives us some confidence that our ownership data is reasonably complete.
...
stargazer(model5, model5.1, model5.2 
          ...
          , column.labels = c("IPUMS","TR Rental Data", "TR Rental Data")
          , add.lines=list(
            c('Year', 'No','No', 'Yes')
...
As noted above, TrueRoll imputed owners' locations based on the mailing addresses for each property's tax bill. On average, Seattle property owners that live in Washington state charge slightly lower rents than owners in the Seattle metropolitan area. Unconditionally, owners that live outside Washington state charge \$7,700 per year more than owners that live in Washington, but outside of the Seattle metropolitan area. As with the owner types analysis above, differences across owner locations become insignificant once you account for unit and building attributes, and census block.
...
TrueRoll's core business mode is based on determining owner-occupancy status of real property. Using TrueRoll's proprietary owner-occupancy flag, we investigated whether owner-occupied properties charge lower rents than properties that do not have an owner living in it. Unconditionally (1), rents in properties where one unit is owner-occupied are about \$2100 lower on average annually than those in properties without an owner-occupied unit. Conditional on covariates (2), this difference converges to zero.
...
The rental market is fairly efficient. As a result, there is much less heterogeneity across rental units than there is across single-family homes. Most of the variation in rental prices can be attributed to a very limited set of explanatory variables: location, bedroom and bathroom count, and building amenities, which are usually highly correlated with the building's effective age. A random sample of 10% of rental units in Seattle, stratified by geography and building class, will likely be sufficient to develop a highly accurate understanding of rental trends in the city. 

The manner in which this data is collected will be extremely consequential to its usefulness. Skeuomorphic web forms should be avoided. Instead, a step-by-step interactive web application should be  used to guide users through a submission process that populates a well-defined data model. The Cook County Assessor's Office recently launched such a platform called the Real Property Income and Expense Online Form. A detailed description of that platform's design [can be found online](https://gitlab.com/ccao-data-science---modeling/documentation/wiki_content/-/blob/master/RPIE/RPIE%20Overview.md).

Randomly sampled rental data, combined with a well-conceived real property database, can be used to predict rents across the City with a high degree of accuracy. Automated valuation models are commonly used in the property tax assessment industry to predict the value of non-rental residential property in a jurisdiction. The exact same approach may be used to model apartment rents. This reduces the amount of rental data that must be collected by a large margin while still allowing a clear picture of rental rates in the City of Seattle. 

#### Compensate landlords for their data

Data is valuable. Laws that compel people to submit data often result in low compliance rates and poor quality data. We suggest that the City of Seattle pay landowners to submit their data. Compensation can be based on our prior knowledge of landlord's portfolio sizes. Coupled with our recommendation above to sample only a fraction of the total rental units, this recommneded payment would result in a modest annual outlay of funds. 

## Continuous investment in capacity

This study is the third of such kind since 2016. In [their 2018 study](https://www.seattle.gov/Documents/Departments/CityAuditor/auditreports/UWSRHSFINAL.pdf), Professor Crowder suggests building a research consortium. We reiterate that recommendation. The current approach of sporadically commissioning studies that rely on bespoke and privileged data sources will produce inconsistent results. If policymakers want to craft well-conceived polices to improve Seattle's housing market, they need to invest in a long-term housing monitoring project. A consortium of researchers from both academic, as well as the private and public sectors, will ensure that policymakers have reliable information upon which to base legislation. Such a consortium could not only assemble and curate common data sets, but they could also build technological tools to collect novel data.
